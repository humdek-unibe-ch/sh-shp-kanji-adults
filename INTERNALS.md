...
The task stylesheet is served by the `kanji-task-style-{1..4}` sections — a
`markdown` style at position 0 on the task page, holding the sheet wrapped in a
`<style>` tag. The labJS section sits after it at position 10, and it is edited
in the CMS like any other section.
...
so the range has to move with the calendar. It is generated when the migration
is built — current year back seventeen — rather than written out, because a
literal list is wrong by the next January. **Rebuild the migration if the study
is still recruiting a year from now.**

`Demo_25` / `Demo_26` (the two adult birth years) are a fixed 1930–2012 and do
accept a parent born in 2012. Qualtrics validated neither, so there is no
original to match; worth a study-owner decision.
